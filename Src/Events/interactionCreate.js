...
module.exports = async (client, interaction) => {
  ...
  if (interaction.guild.id === GUILDID) {
    if (interaction.channel.id === CHTICKET) {
      if (interaction.isSelectMenu() && interaction.customId === "tickets") {
        if (interaction.values[0] === "tk2") {
            const ticketDoc2 = await TSchema.findOne({
                guildID: interaction.guild.id,
                userID: interaction.user.id,
            });

    if (ticketDoc2) {
      const channel = interaction.guild.channels.cache.get(ticketDoc2.channelID);
            if (channel) {
              interaction.reply({
                  embeds: [ new MessageEmbed()
                      .setDescription(`__Error__ You already have an open ticket`)
              ...
              await ticketDoc2.deleteOne();
              guildDoc.ticketCount += 1;
              await guildDoc.save();
              let everyone = interaction.guild.roles.cache.find((m) => m.name == "@everyone");
              const ticketChannel = await interaction.guild.channels.create(`🎫・ticket・${guildDoc.ticketCount}`, {
                  type: "text",
                  parent: PARENTID,
                  permissionOverwrites: [{
                      id: everyone.id, deny: ["VIEW_CHANNEL", "SEND_MESSAGES", "ADD_REACTIONS"]  },
              ...
              const msg = await ticketChannel.send({ content: `${interaction.user} __Ticket Creado__ \nTicket nº\`${guildDoc.ticketCount}\``
            })
              const tickDoc2 = new TSchema({
                guildID: interaction.guild.id,
                userID: interaction.user.id,
                channelID: ticketChannel.id,
                msgID: msg.id,
